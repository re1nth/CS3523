...
void* reader(void* arg)
{
	...
	for(int i=0;i<r_count;i++)
	{
		...
		sem_wait(out);
		ctrout++;
		if(swait==1 && ctrin==ctrout) sem_post(wrt);
		sem_post(out);
...
int main()
{
	...
	fp1=fopen("FRW-log.txt","w");
	fp2=fopen("FRW-Average_time.txt","w");

	in=sem_open(semname1,O_CREAT|O_EXCL,0644,1);
	out=sem_open(semname2,O_CREAT|O_EXCL,0644,1);
	wrt=sem_open(semname3,O_CREAT|O_EXCL,0644,0);
